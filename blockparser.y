...
loop
	: LOOP IDENTIFIER ':' BEG stmt_list END {$$ = new ASTstmt(TLOOP,$5,NULL,$2);}
	| LOOP ':' BEG stmt_list END	    	 {$$ = new ASTstmt(TLOOP,$4,NULL,"");}
	;

mif
	: IF condition ':' BEG stmt_list END ELSE ':' BEG stmt_list END	{
		                                                            $$ = new ASTif($2,$5);
		                                                            auto else_node = new ASTif(NULL,$10);
		                                                            $$->tail = else_node;
		                                                            else_node->tail = NULL;
	                                                             }
	| IF condition ':' BEG stmt_list END ELIF condition ':' BEG stmt_list END eliftstmt {
		       		                                                                 $$ = new ASTif($2,$5);
		                                                                             auto else_node = new ASTif($8,$11);
		                                                                             $$->tail = else_node;
		                                                                             else_node->tail = $13;
	                                                                               }
	| IF condition ':' BEG stmt_list END  {$$ = new ASTif($2,$5);}
	;
...
eliftstmt
	: ELSE ':' BEG stmt_list END 			 {$$ = new ASTif(NULL,$4);}
    | %empty 							 {$$ = NULL;}
	| ELIF condition ':' BEG stmt_list END eliftstmt {$$ = new ASTif($2,$5);$$->tail=$7;}
	;
