        ...
        switch(stmt->type){
                ...
                case TPC:
                        {
                          ...
                          stmt->expr->f -> nesting_diff = currentScope->nestingLevel - s->nestingLevel;      
                ...
                case TRET:
                        {
                           ...
                           if(!equalType(_TRET_TYPE,s->u.eFunction.resultType) &&  
                              !(equalType(_TRET_TYPE,typeInteger) && equalType(s->u.eFunction.resultType,typeChar)) &&
                              !(equalType(_TRET_TYPE,typeChar)    && equalType(s->u.eFunction.resultType,typeInteger)) ) {
                ...
                case TASSIGN:
                {
                       ...
                       if(!equalType(lval_type,rval_type)){
                        ...
                        stmt->lvalue-> nesting_diff = currentScope->nestingLevel - s->nestingLevel;      
        ...
        switch(expr->op){
                case '+':
                        if( (!equalType(typeInteger,right) && !equalType(typeChar,right)) ||
                            (!equalType(typeInteger,left) && !equalType(typeVoid,left) && !equalType(typeChar,left)) ||
                ...
                case 'f':
                        {
                          ...
                          if(par_vector_ptr == NULL){ 
                            if(par == NULL) return a->u.eFunction.resultType;
                            else {
                              printf("%s:\n", a -> id);
                              error("\rfewer parameter provided in function"); 
                              exit(1);
...
// ####################################################################################################### \\
    ...
    /* -----------------------------------------------------------------------------
       ...
       ----------------------------------------------------------------------------- */
        ...
        ASTheader *writeInteger_header = new ASTheader(typeVoid, first_Param, "writeInteger");
        ...
        ASTheader *writeByte_header = new ASTheader(typeVoid, first_Param, "writeByte");
        ...
        ASTheader *writeChar_header = new ASTheader(typeVoid, first_Param, "writeChar");
        ...
        ASTheader *writeString_header = new ASTheader(typeVoid, first_Param, "writeString");
    ...
    /* -----------------------------------------------------------------------------
       ...
       ----------------------------------------------------------------------------- */
        ...
        ASTheader *readInteger_header = new ASTheader(typeInteger, NULL, "readInteger");
        ...
        param_identifiers = new vector<string>(); 
        param_identifiers->push_back("s");
        second_Param = new ASTparam(param_identifiers, typeIArray(typeChar) , NULL); //////WHY typeIArray ???????

        param_identifiers = new vector<string>(); 
        param_identifiers->push_back("n");
        first_Param  = new ASTparam(param_identifiers, typeInteger, second_Param);

        ASTheader *readString_header = new ASTheader(typeVoid, first_Param, "readString");
    ...
    /* -----------------------------------------------------------------------------
       ...
       ----------------------------------------------------------------------------- */
        ...
        ASTheader *extend_header = new ASTheader(typeInteger, first_Param, "extend");
    ...
    /* -----------------------------------------------------------------------------
       ...
       ----------------------------------------------------------------------------- */
        ...
        ASTheader *strlen_header = new ASTheader(typeInteger, first_Param, "strlen");
        ...
        first_Param  = new ASTparam(param_identifiers, typeIArray(typeChar), second_Param);

        ASTheader *strcmp_header = new ASTheader(typeInteger, first_Param, "strcmp");
